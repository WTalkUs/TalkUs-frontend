"use client";
import { Avatar, Button, Textarea, Spinner, Alert, Dropdown, DropdownTrigger, DropdownMenu, DropdownItem } from "@heroui/react";
...
import { ThumbsUp, ThumbsDown, MoreVertical, Edit, Trash, MessageSquare, ChevronDown, ChevronUp } from "lucide-react";
        ...
        return commentsList.map(c => {
          if (c.CommentID === commentId) {
            ...
            if (currentReaction === reaction) {
              ...
              return { ...c, Likes: likes, Dislikes: dislikes, userReaction: null };
            }
            ...
            return { ...c, Likes: likes, Dislikes: dislikes, userReaction: reaction };
          }
...
useEffect(() => {
  const loadComments = async () => {
    const res = await commentService.getByPost(); 
    if (res.success) {
  ...
  <div 
    ...
    className={`border-b border-gray-100 pb-6 last:border-0 ${depth > 0 ? 'ml-10 pl-6 border-l-2 border-gray-200' : ''}`}
  >
  ...
  `}
            ...
            {hasReplies && (
              <Button 
                ...
                variant="ghost" 
                size="md"
                startContent={comment.showReplies ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
                onPress={() => toggleReplies(comment.CommentID)}
              >
                {comment.showReplies ? 'Ocultar respuestas' : `Mostrar respuestas (${comment.replies?.length})`}
              </Button>
            )}
          </>
        )}
