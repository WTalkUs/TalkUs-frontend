...
type CommentResponse<T = Comment | Comment[]> =
  | CommentSuccessResponse<T>
...
function normalizeComment(comment: any, currentUserId: string): Comment {
  ...
  return {
    ...
    userReaction,
    Likes: Object.values(comment.reactions || {}).filter((r) => r === "like")
      .length,
    Dislikes: Object.values(comment.reactions || {}).filter(
      (r) => r === "dislike"
    ).length,
    replies: (comment.replies || []).map((r: any) =>
      normalizeComment(r, currentUserId)
    ),
  };
}
export const commentService = {
  ...
  async react({
    ...
    try {
      ...
      return {
        success: true,
        data: normalizeComment(response.data, user.uid),
...
function handleCommentError(
  ...
  if (axios.isAxiosError(error)) {
    ...
    const data = axiosError.response?.data as
      ...
      | undefined;
    const errorMessage =
      data?.error || data?.message || `Error al ${action} el comentario`;
